...
- Adds `geocode_file()` and `geocode_file_job()` which geocode a file of addresses using the `BatchGeocode` job endpoint. Results are written with `write_geocode_result()` and read with `read_geocode_result()`.
